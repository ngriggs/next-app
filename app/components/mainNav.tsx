...
export function MainNav({ items }: MainNavProps) {
  return (
    <div className="fixed left-[calc(-50vw_+_50%)] top-0 z-50 flex h-[80px] w-screen gap-6 bg-white md:gap-0">
      <div className="mx-auto flex max-w-7xl flex-1 px-1 sm:px-0">
        <div className="my-auto grid justify-items-start sm:flex sm:flex-1">
          <Link href="/" className="hover:cursor-pointer">
            <Image
              width={200}
              height={200}
              alt="logo"
              className=" h-12 hover:translate-y-[-1px] md:ml-0 "
        ...
        {items?.length ? (
          <nav className="my-auto hidden justify-center space-x-8 md:flex md:flex-1 ">
            {items?.map((item, index) => (
              <Popover key={index}>
                <PopoverTrigger className="group flex cursor-pointer flex-row text-lg font-medium text-[#232323] hover:underline hover:underline-offset-[8px]">
                  {item.title}
                  <Icons.chevronDown
                ...
                >
                  {item.subheadings?.map((sub) => (
                    ...
                    >
                      <ul className="ml-0">
                        <li className="text-base font-normal hover:underline hover:underline-offset-2">
                          {sub.name}
                        </li>
                      </ul>
                    </Link>
